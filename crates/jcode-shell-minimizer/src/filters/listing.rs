...
/// Summarize well-known dependency manifests (`Cargo.toml`, `package.json`,
/// `go.mod`) into a compact name + dependency-count + first-deps digest.
/// Returns `None` for any other file so the caller falls through to the
/// source-outline path.
...
mod tests {
	...
	#[test]
	fn summarizes_cargo_manifest_from_default_cat() {
		let cfg = MinimizerConfig { enabled: true, ..Default::default() };
		let ctx = ctx_command("cat", "cat Cargo.toml", &cfg);
		let input = "[package]\nname = \"rtk\"\n\n[dependencies]\nclap = { version = \"4\", \
		             features = [\"derive\"] }\nanyhow = \"1.0\"\nserde_json = \"1\"\n";
		let out = filter(&ctx, input, 0);
		assert!(out.text.contains("rtk"), "package name kept: {}", out.text);
		assert!(out.text.contains("dependencies: 3"), "dep count: {}", out.text);
		assert!(out.text.contains("clap"), "deps listed: {}", out.text);
		assert_ne!(out.text, input, "manifest must be summarized");
	}
	...
	#[test]
	fn outlines_large_source_from_default_cat() {
		let cfg = MinimizerConfig { enabled: true, ..Default::default() };
		...
		input.push_str("}\nfn main() -> Result<()> {\n    Ok(())\n}\n");
		let out = filter(&ctx, &input, 0);
		assert_ne!(out.text, input, "≥160-line source must outline at Default");
		assert!(out.text.len() < input.len(), "outline shrinks: {} vs {}", out.text.len(), input.len());
		assert!(out.text.contains("struct Cli") || out.text.contains("fn main"), "declarations kept: {}", out.text);
	}
	...
	#[test]
	fn default_level_outlines_large_source_cat() {
		// lex parity: Default level outlines `cat` of source files once they
		// reach the 160-line / 12 KiB threshold — no Aggressive gate.
		let cfg = MinimizerConfig { enabled: true, ..Default::default() };
		let ctx = ctx_command("cat", "cat src/main.rs", &cfg);
		let mut input = String::from("use anyhow::Result;\nmod cargo_cmd;\n\nstruct Cli {\n");
		for idx in 0..180 {
		...
		input.push_str("}\nfn main() -> Result<()> {\n    Ok(())\n}\n");
		let out = filter(&ctx, &input, 0);
		assert!(out.changed, "default level must outline a 184-line file");
		assert!(out.text.contains("use anyhow::Result;"), "{:?}", out.text);
		...
		assert!(!out.text.contains("field_179"), "elided field must not survive");
	}
	...
	#[test]
	fn summarizes_package_json_from_cat() {
		let cfg = MinimizerConfig { enabled: true, ..Default::default() };
		let ctx = ctx_command("cat", "cat package.json", &cfg);
		let input = concat!(
			"{\n",
			"  \"name\": \"demo\",\n",
			"  \"dependencies\": { \"express\": \"^4.18.2\" },\n",
			"  \"devDependencies\": { \"typescript\": \"^5.0.0\", \"vitest\": \"\" }\n",
			"}\n",
		);
		let out = filter(&ctx, input, 0);
		assert_eq!(
	...
	#[test]
	fn summarizes_go_mod_from_cat() {
		let cfg = MinimizerConfig { enabled: true, ..Default::default() };
		let ctx = ctx_command("cat", "cat go.mod", &cfg);
		let input = concat!(
	...
	#[test]
	fn non_manifest_non_source_cat_passthrough_at_default() {
		let cfg = MinimizerConfig { enabled: true, ..Default::default() };
		let ctx = ctx_command("cat", "cat README.md", &cfg);
		let input = numbered_lines(200);
		let out = filter(&ctx, &input, 0);
		assert_eq!(out.text, input, "non-source, non-manifest cat stays raw");
	}
