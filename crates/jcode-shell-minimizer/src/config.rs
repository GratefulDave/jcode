...
pub struct MinimizerOptions {
	...
	/// `JCODE_MINIMIZER_LEGACY_FILTERS` environment variable (truthy = "1",
...
pub struct MinimizerConfig {
	...
	/// `MinimizerOptions.legacy_filters` or the `JCODE_MINIMIZER_LEGACY_FILTERS`
...
impl MinimizerConfig {
	...
	pub fn from_options(opts: &MinimizerOptions) -> Self {
		...
		let legacy_requested = resolve_legacy_filters(
			opts.legacy_filters,
			std::env::var("JCODE_MINIMIZER_LEGACY_FILTERS")
				.ok()
				.as_deref(),
		);
			...
			if let Ok(contents) = fs::read_to_string(&expanded) {
					...
					&& !expected.is_empty()
				{
					...
					if !actual_hex.eq_ignore_ascii_case(expected) {
						eprintln!(
							"[jcode-shell-minimizer] settings_hash mismatch for {} (expected {}, got {}); \
							 ignoring file",
							expanded.display(),
							expected,
							actual_hex
				...
				match pipeline::parse_file(&contents, "user") {
					...
					Err(err) => {
						eprintln!("[jcode-shell-minimizer] user filters: {err}");
					},
				}
			}
		}
...
impl SettingsFile {
	fn merge_into(self, cfg: &mut MinimizerConfig) {
		...
		{
			eprintln!(
				"[jcode-shell-minimizer] unsupported schema_version {v} in settings file (expected \
...
/// and the raw `JCODE_MINIMIZER_LEGACY_FILTERS` env value.
///
